...
- [#1138](https://github.com/genomic-medicine-sweden/nallo/pull/1138) - Refactored `CALL_SVS` into per-caller subworkflows (`SNIFFLES_SV`, `SNIFFLES1_SV`, `SEVERUS_SV`, `DEBREAK_SV`, `HIFICNV_SV`, `SAWFISH_SV`); `CALL_SVS` now emits `sv_calls_raw` (2-tuple, VCF only, for callers that need VEP prep) and `sv_calls_indexed` (3-tuple, for callers that pre-index); `VEP_PREP_SV`, `BCFTOOLS_SORT`, and the optional region `BCFTOOLS_VIEW` filter moved to `nallo.nf`; `needs_reheader` and `skip_vep_prep` flags stamped on `meta` by each per-caller subworkflow
